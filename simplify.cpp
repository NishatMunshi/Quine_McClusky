...
Column construct_first_column(const std::list<unsigned> &_terms, const unsigned _numberOfVariables)
{
    ...
    numbersWithNumberOfOnesInThem.sort([](const std::pair<unsigned, unsigned> &_x, const std::pair<unsigned, unsigned> &_y)
...
auto get_essential_primeImplicant_indices(const std::vector<std::vector<bool>> &_essentialPrimeImplicantTable)
{
    ...
    while (true)
    {
        ...
        contributors.sort([](const contributor &_x, const contributor &_y)
                          { return _x.index < _y.index; });
        contributors.unique([](const contributor &_x, const contributor &_y)
                            { return _x.index == _y.index; });
        ...
        contributors.sort([](const contributor &_x, const contributor &_y)
                          { return _x.numberOfContributionsTowardNotIncludedMinTerms > _y.numberOfContributionsTowardNotIncludedMinTerms; });
        ...
        contributors.sort([](const contributor &_x, const contributor &_y)
                          { return _x.numberOfContributionsOverall > _y.numberOfContributionsOverall; });
...
int main()
{
    ...
    std::cout << "The simplified function is: \n\t" << function;
